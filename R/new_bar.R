new_bar <- function(optim, eff, y, x, z, p0, p1) {
  ...
  names(n) <- c("effective_n", "n0", "n1")

  bar <- structure(
    list(),
    class = "bar",
    ts = list(y_eff = y, x = x, z_del = zdrH$z, H = zdrH$H,
              R = R, res = fit$res),
    est = list(d = zdrH$d, r = zdrH$r, coe = coe, resvar = rv),
    orders = orders,
    rss = fit$rss,
    n = n
    )
...
get_zdrH <- function(optim, eff, z) {
  ...
  f     <- function(z, r) ts_hys(z, r[1], r[2])
...
coe_to_matrix <- function(coe, p0, p1) {
  ...
  zeros0 <- rep(0, nc - (p0 + 1))
  zeros1 <- rep(0, nc - (p1 + 1))
